...
const COMMANDS = ['pl_pause', 'seek', 'pl_forcepause', 'pl_forceresume']
...
module.exports = function(options) {
  ...
  return {
    ...
    play(callback) {
      return sendCommand(address, 'pl_forceresume', callback)
    },
    pause(callback) {
      return sendCommand(address, 'pl_forcepause', callback)
    },
    getStat(callback) {
    ...
    startPoll() {
      ...
      timerId = setInterval(() => {
        return send(address, (err, stat) => {
          ...
          if (state !== stat.state) {
            state = stat.state
            return event.emit('STATE_CHANGE', state)
          }
        })
      }, opt.pollInterval)
